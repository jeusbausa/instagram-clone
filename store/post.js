  ...
  initPosts: async (context, payload) => {
    ...
    try {
      const result = await $nuxt.$fire.firestore
        .collection('users')
        .where('id', '==', rootState.auth.user.uid)
        .get()
      const [userFollowed] = result.docs.map((item) => ({
        ...item.data(),
        docId: item.id,
      }))
      const posts = await $nuxt.$fire.firestore
        .collection('posts')
        .where('userId', 'in', userFollowed.following)
        .get()
  ...
  addPost: async (context, payload) => {
    ...
    try {
      ...
      const url = await $nuxt.$fire.storage
        ...
        .put(photo)
      await $nuxt.$fire.firestore.collection('posts').add({
        userId: rootState.auth.user.uid,
        photo: await url.ref.getDownloadURL(),
        caption,
        location,
        createdAt: Date.now(),
      })
