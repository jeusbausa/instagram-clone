export const actions = {
  login: async (context, credentials) => {
    ...
    try {
      await $nuxt.$fire.auth.signInWithEmailAndPassword(
        credentials.email,
        credentials.password
      )
      const token = await $nuxt.$fire.auth.currentUser.getIdToken(
        /* forceRefresh */ true
      )
  ...
  signUp: async (context, credentials) => {
    ...
    try {
      const blobImage = await fetch(
        'https://firebasestorage.googleapis.com/v0/b/instagram-clone-tailwindcss.appspot.com/o/users%2Favatar.jpg?alt=media'
      )
      const result = await $nuxt.$fire.auth.createUserWithEmailAndPassword(
        credentials.email,
        credentials.password
      )
      ...
      await result.user.updateProfile({
        ...
        photoURL: await url.ref.getDownloadURL(),
      })
